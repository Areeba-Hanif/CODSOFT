...
public:
  ...
  void bookReturn(string name, string auth, string isbn) {

    Book *current = head;
    while (current != NULL) {

      if (current->title == name && current->author == auth &&
          current->ISBN == isbn) {
        if (!current->isAvailable) {

          current->isAvailable = true;
          cout << "Book '" << current->title << "' is returned." << endl;
          return;
        } else {
  ...
  void calculateFine(string name, string auth, string isbn, int daysOverdue) {
    ...
    while (current != NULL) {
      if (current->title == name && current->author == auth &&
        ...
        cout << "Fine for " << current->title << " is Rs  " << fineAmount << "." << endl;
        return;
      }
      current = current->next;
    }
  ...
  // function to store all the Book Information in the File "database.txt"
  ...
  void storeBooksToFile(const char filename[100]) {
    ...
    file.close();
    cout << "Book stored in the file." << endl;
  }




  // Function to display all Books from  file
  ...
  void sortBooks() // Function to Sort all the Books in Alphabetical order
  {
    bool swapped = true;
    while (swapped) {
      swapped = false;
  ...
  void borrowerMenu() {
    ...
    cout << "\n\t\t 4 . Checkout  Book" << endl;
    cout << "\n\t\t 5 . Return Book" << endl;
    cout << "\n\t\t 6 . Display Available Books" << endl;
  }
    ...
    if (choice == 1) {
      ...
      do {
        ...
        case 5:
          ...
          book.bookReturn(t, a, i);
          book.calculateFine(t, a, i, d);
